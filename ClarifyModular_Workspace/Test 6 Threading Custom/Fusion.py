...
class FusionWorker(QThread):
    ...
    def set_data(self, boxes, frame_count, recognizer, frame):
        self.boxes = boxes
    ...
    def run(self):
        while self.running:
            if self.boxes is not None and self.frame_count is not None and self.recognizer is not None and self.frame is not None:
                results = self.tracker.update(self.boxes, self.frame_count, self.recognizer, self.frame)
                self.fusion_done.emit(results)
                self.boxes = None
                self.frame_count = None
                self.recognizer = None
                self.frame = None
